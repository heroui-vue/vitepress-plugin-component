...
<template>
  <div class="vp-component-tabs">
    <header>
      <template
        v-for="({ label, value }) in [{ label: '预览', value: 'preview' }, { label: '源代码', value: 'code' }] as const"
        :key="value"
      >
        <input
          :id="`tab-${value}`"
        ...
        >
        <label :for="`tab-${value}`" @click="activeTab = value">{{ label }}</label>
      </template>
    </header>
    ...
    <section v-else-if="activeTab === 'code'">
      <component :is="highlightVue.component" language="js" :code="sourceCode" />
    </section>
  </div>
</template>
